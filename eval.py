...
if __name__ == '__main__':
    ...
    if ep.lol:
        ...
        if ep.DICM:
            eval_data = DataLoader(dataset=get_SICE_eval_set("./datasets/Five unpaired datasets/DICM"), num_workers=num_workers, batch_size=1, shuffle=False)
            output_folder = './output/DICM_syn/'
        elif ep.LIME:
            eval_data = DataLoader(dataset=get_SICE_eval_set("./datasets/Five unpaired datasets/LIME"), num_workers=num_workers, batch_size=1, shuffle=False)
            output_folder = './output/LIME_syn/'
        elif ep.MEF:
            eval_data = DataLoader(dataset=get_SICE_eval_set("./datasets/Five unpaired datasets/MEF"), num_workers=num_workers, batch_size=1, shuffle=False)
            output_folder = './output/MEF_syn/'
        elif ep.NPE:
            eval_data = DataLoader(dataset=get_SICE_eval_set("./datasets/Five unpaired datasets/NPE"), num_workers=num_workers, batch_size=1, shuffle=False)
            output_folder = './output/NPE_syn/'
        elif ep.VV:
            eval_data = DataLoader(dataset=get_SICE_eval_set("./datasets/Five unpaired datasets/VV"), num_workers=num_workers, batch_size=1, shuffle=False)
            output_folder = './output/VV_syn/'
        elif ep.custome:
